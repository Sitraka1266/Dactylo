...
io.on("connection", (socket) => {
  ...
  socket.on("finished", (data) => {
    ...
    if (!rooms[roomId] || rooms[roomId].finishedPlayers.includes(clientId))
      return;
    rooms[roomId].finishedPlayers.push(clientId);

    const stats = rooms[roomId].players[clientId].stats || {
      cpm: 0,
      errors: 0,
    };
    ...
    if (
      ...
      rooms[roomId].finishedPlayers.length
    ) {
      let maxCPM = 0;
      let winnerId = null;
...
server.listen(3000, () =>
  console.log("Serveur en ligne sur http://0.0.0.0:3000")
);
